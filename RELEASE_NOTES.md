# Release notes — fabric-aiops 0.7.0
...
- **CLI writes now exit non-zero on a governed refusal** (they printed the error but exited 0, so a CI `$?` check read a refused write as success).
