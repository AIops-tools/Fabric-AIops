...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/fabric-aiops` to
  `@zw008/fabric-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/fabric-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/fabric-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install fabric-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/fabric-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [fabric-aiops, uvx]` — either one suffices — with every variable kept
...
- Docs: records what a real standalone UniFi Network Application 10.4.57 shows about authentication — the classic endpoints are real under cookie auth, but the `X-API-KEY` Integration API 404s there and keys cannot be minted headlessly. **No code change**: the fix direction is still unproven without a UniFi OS console.
