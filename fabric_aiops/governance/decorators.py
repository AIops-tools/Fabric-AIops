"""The ``@governed_tool`` decorator — mandatory wrapper for all Fabric MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.fabric-aiops/audit.db``
...
from fabric_aiops.governance.audit import detect_agent, get_engine
from fabric_aiops.governance.budget import BudgetExceeded, get_budget
from fabric_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from fabric_aiops.governance.patterns import PatternMatch, get_pattern_engine
from fabric_aiops.governance.policy import PolicyResult, get_policy_engine
from fabric_aiops.governance.readonly import is_read_only, read_only_denial
from fabric_aiops.governance.sanitize import sanitize

_log = logging.getLogger("fabric-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all Fabric MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.fabric-aiops/undo.db and the
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`fabric_aiops.governance.outcome`). The change may be live, so an
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight network-fabric calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``fabric_aiops.ops.jobs`` → ``fabric-aiops``
    ``mcp_server.server`` → ``fabric-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("fabric_aiops") or module.startswith("mcp_server"):
        return "fabric-aiops"
    return "unknown"
