...
2. **Runaway breaker (on by default, disable with ``FABRIC_RUNAWAY_MAX=0``).**
   The same ``(tool, params)`` called more than ``FABRIC_RUNAWAY_MAX`` times
   within ``FABRIC_RUNAWAY_WINDOW_SEC`` trips a short cooldown. Catches a tight
