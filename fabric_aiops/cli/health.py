...
def health_uplink_rca(
    ...
    print_result(
        ops.uplink_loss_and_latency_rca(
            records, loss_pct=loss_pct, latency_ms=latency_ms, **limit_kwargs(limit)
        )
    )
...
def health_score(
    ...
    print_result(
        ops.network_health_score(device_rows, uplinks=uplinks, **limit_kwargs(limit))
    )
