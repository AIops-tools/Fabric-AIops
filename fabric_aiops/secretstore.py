"""Encrypted secret storage for fabric-aiops.

Secrets (Meraki API keys) are NEVER written to disk in plaintext. They
live in ``~/.fabric-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``FABRIC_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "fabric-aiops"
CONFIG_DIR = Path.home() / ".fabric-aiops"
MASTER_PASSWORD_ENV = "FABRIC_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``FABRIC_<TARGET>_APIKEY``; ``prefix='FABRIC_'`` and
    ``suffix='_APIKEY'`` map them back to a target name. The plaintext
