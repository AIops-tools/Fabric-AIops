...
from fabric_aiops.ops._util import clean, clean_list, op_get, op_get_pages
...
def list_clients(conn: Any, network_id: str, timespan: int = 86400) -> list[dict]:
    ...
    return clean_list(
        op_get_pages(conn, "clients.list", params={"timespan": span}, network_id=network_id)
    )


def get_client(conn: Any, network_id: str, client_id: str) -> dict:
    """[READ] One client's detail (description, MAC, IP, VLAN, manufacturer)."""
    return clean(op_get(conn, "clients.get", network_id=network_id, client_id=client_id))
...
def client_usage(conn: Any, network_id: str, client_id: str) -> dict:
    ...
    """
    rows = clean_list(
        op_get(conn, "clients.usage", network_id=network_id, client_id=client_id)
    )
...
def client_connectivity(conn: Any, network_id: str, client_id: str) -> dict:
    ...
    """
    stats = clean(
        op_get(conn, "clients.connectivity", network_id=network_id, client_id=client_id)
    )
