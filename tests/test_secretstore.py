...
nothing touches the real ``~/.fabric-aiops``.
"""
...
def test_migrate_legacy_env(store_dir):
    (store_dir / ".env").write_text(
        "FABRIC_ORG1_APIKEY=legacy-key\n# comment\nFOO=bar\n"
    )
    imported = ss.migrate_legacy_env("FABRIC_", "_APIKEY", "pw")
    assert imported == ["org1"]
    assert ss.SecretStore.unlock("pw").get("org1") == "legacy-key"
