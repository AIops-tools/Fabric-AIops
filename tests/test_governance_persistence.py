...
throwaway home (``FABRIC_AIOPS_HOME``) and assert that the rows compliance
evidence is built from actually land on disk, that a high-risk write runs
without any approver (the skill authorizes nothing), and that an approver, when
supplied, is merely recorded.
"""
...
from fabric_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("FABRIC_AUDIT_APPROVED_BY", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="prod-fabric")
...
def test_high_risk_runs_without_an_approver_and_is_audited(gov_home):
    ...
    result = _drop_widget(name="w2", target="prod-fabric")
...
def test_an_approver_is_still_recorded_when_supplied(gov_home, monkeypatch):
    ...
    monkeypatch.setenv("FABRIC_AUDIT_APPROVED_BY", "netops-alice")
    result = _drop_widget(name="w3", target="prod-fabric")
    assert result["status"] == "dropped"
...
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: update_device must capture
    the changed keys' prior values before the PUT and persist the restore-undo
    on disk."""
    monkeypatch.setenv("FABRIC_AUDIT_APPROVED_BY", "netops-alice")
    conn = MagicMock(name="conn")
    conn.get.return_value = {"serial": "Q2AB-CDEF-GHIJ", "name": "old-name", "tags": ["a"]}
    ...
    result = gov.update_device(serial="Q2AB-CDEF-GHIJ", attrs={"name": "new-name"})
    ...
    assert len(undo) == 1
    assert undo[0]["undo_id"] == result["_undo_id"]
    assert undo[0]["undo_tool"] == "update_device"
    assert "old-name" in undo[0]["undo_params"]

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["update_device"]
