...
@governed_tool(risk_level="low")
@tool_errors("dict")
def network_alerts(network_id: str, target: Optional[str] = None) -> dict:
    ...
    Args:
        ...
        target: Target name from config; omit for the default.
    """
    return ops.network_alerts(_get_connection(target), network_id)
...
def network_traffic(
    ...
    Args:
        ...
        target: Target name from config; omit for the default.
    """
    return ops.traffic_summary(_get_connection(target), network_id, timespan)
